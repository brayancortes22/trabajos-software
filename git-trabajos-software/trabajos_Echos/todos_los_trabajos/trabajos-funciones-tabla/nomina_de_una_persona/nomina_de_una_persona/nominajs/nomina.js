...
// se llama a las funciones y se les asigna en una variable
    ...
    pago_personas=[{id,numeroDoc,nombre_p,apellidos_p,edades,
        precioSalario,precio_arl,precio_salud,precio_pension,precio_subtrasporte,precio_retencion,
        precio_extra,precio_deducible,precio_pagototal }]

        console.table(pago_personas);
       
    }
   
// Definir una variable para almacenar el total de los salarios
let totalSalarios = 0;
// Bucle para recorrer la lista de personas en la nómina
for (contador = 0; contador < nomina.length; contador++) {
    valores = nomina[contador].valorDias;
    dia = nomina[contador].diastrabajados;
    
    totalSalarios += precio_pagototal;
    //'precio_pagototal' contiene el salario total de la persona
}
// Mostrar el total de todos los salarios sumados
console.log('El total de todos los salarios sumados es:'+totalSalarios);
